...
std::size_t popcount_bitset(const Bitset &bitset) {
    return std::accumulate(
        bitset.begin(), bitset.end(), static_cast<std::size_t>(0),
        [](std::size_t total, std::uint64_t word) {
#if defined(__GNUC__) || defined(__clang__)
            return total + static_cast<std::size_t>(__builtin_popcountll(word));
#else
            std::size_t count = 0;
            std::uint64_t x = word;
            while (x != 0) {
...
void FilterView::apply_position_constraints(std::string_view guess,
    ...
    for (std::size_t pos = 0; pos < m_index.m_word_length; ++pos) {
        ...
        if (mark != BitsetIndex::k_green && mark != BitsetIndex::k_yellow &&
            ...
            throw std::invalid_argument(
                "feedback must use only 'g', 'y', or 'b'");
        }
        if (mark == BitsetIndex::k_green || mark == BitsetIndex::k_yellow) {
        ...
        if (mark == BitsetIndex::k_green) {
            bit_and(
                m_candidates,
                m_index.m_position_letter_masks[pos][static_cast<std::size_t>(
                    letter)]);
        } else {
            bit_and_not(
                m_candidates,
...
void FilterView::apply_count_constraints(const letter_count_array &min_count,
    ...
    for (std::size_t letter = 0; letter < BitsetIndex::k_alphabet_size;
        ...
        const Bitset allowed = bit_or(m_index.m_letter_count_masks[letter],
                                      min_c, max_c + 1, m_candidates.size());
